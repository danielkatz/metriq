...
for (const cardinality of CARDINALITIES) {
    ...
    app.get(PROM_CLIENT_PATH, async (req, res) => {
        res.type(pcRegistry.contentType);

        const metrics = await pcRegistry.metrics();
...
// Benchmark
describe.each(CARDINALITIES)("Fastify (cardinality=%d)", (cardinality) => {
    bench(
        "metriq",
        async () => {
